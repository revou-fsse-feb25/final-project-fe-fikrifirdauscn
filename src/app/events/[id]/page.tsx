...
export default function EventDetailPage() {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        const response = await apiClient.get(`/events/${id}`); // <-- Tidak perlu token lagi
  ...
  const handleBooking = async () => {
    ...
    if (!token) {
      // Jika tidak ada token, simpan URL saat ini dan alihkan ke login
      ...
      <div className="bg-white rounded-lg shadow-xl overflow-hidden md:flex">
        ...
        <div className="p-8 md:w-1/2 flex flex-col justify-between">
          <div>
            ...
            <p className="text-xl text-gray-600 mt-2">{event.artist}</p>
          ...
          <div className="mt-8 border-t pt-4">
            ...
            <button
              onClick={handleBooking} // <-- Logic berada di sini
